...
class UserTest < ActiveSupport::TestCase
  ...
  def setup
    # @error_messages = ActiveRecord::Error.default_error_messages
  ...
  def test_user_invalidity
    ...
    attributes.each do |attribute|
      assert @invalid_user.errors.invalid?(attribute), "#{attribute} is not invalid"
    end
  end
  ...
  def test_email_with_valid_examples
    ...
    valid_emails = valid_endings.collect do |ending|
      "foo.bar_1-9@baz-quux9.example.#{ending}"
    end
  ...
  def test_email_with_invalid_examples
    ...
    invalid_emails.each do |email|
      user.email = email
      assert !user.valid?, "#{email} tests at valid but shouldn't be"
      assert_equal 'must be a valid email address', user.errors.on(:email)
    end
  end
  
  # Test the screen name validator against valid examples
  ...
  # Test the screen name validator against invalid examples
